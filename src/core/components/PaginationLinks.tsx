...
const PaginationLinks = <T,>({ data, goToPage }: PaginationLinksProps<T>): JSX.Element => {
	...
	const [separatorAround, setSeparatorAround] = useState<boolean>(false);
	...
	useEffect(() => {
		if (data !== null) {
			...
			setSeparatorAround(showSeparatorAtAround);
		}
	}, [data]);
	...
	return (
		<div className="d-flex justify-content-between align-items-center flex-wrap ">
			<div className="d-flex justify-content-between align-items-center py-1">
				<div className="pe-2">
					Mostrando del <span>{data.from}</span> al
					<span>{data.to}</span> de un total de
					<span>{data.total}</span> registros
				</div>
				<FormSelect
					size="sm"
					className="w-auto"
